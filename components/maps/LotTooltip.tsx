    ...
    >
      <div className="p-1 font-sans">
        <div className="flex justify-between items-start mb-2 gap-2">
          <h3 className="font-bold text-gray-900 text-base m-0 leading-none">Lote {properties.id || properties.name}</h3>
        ...
        <div className="flex flex-col gap-1 text-sm text-gray-700">
          <p className="m-0"><span className="font-semibold text-gray-500">Superficie:</span> {properties.area}</p>
          {properties.price && (
            <p className="m-0"><span className="font-semibold text-gray-500">Precio:</span> {properties.price}</p>
          )}
        </div>
        
        {(properties.zone || properties.view) && (
          <div className="mt-3 flex flex-col gap-1 text-xs text-gray-500 border-t border-gray-100 pt-2 m-0">
            {properties.zone && <p className="m-0">Zona: {properties.zone}</p>}
            {properties.view && <p className="m-0">Vista: {properties.view}</p>}
          </div>
        )}
      </div>
    </Popup>
